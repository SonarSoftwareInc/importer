...
class TokenizedCreditCardImporter extends AccessesSonar
{
    /**
    ...
    public function import($pathToImportFile)
    {
        if (($handle = fopen($pathToImportFile,"r")) !== FALSE)
        {
            ...
            $failureLogName = tempnam(getcwd() . "/log_output","tokenized_card_import_failures");
            $failureLog = fopen($failureLogName,"w");

            $successLogName = tempnam(getcwd() . "/log_output","tokenized_card_import_successes");
            ...
            $requests = function () use ($validData)
            {
                ...
                {
                    yield new Request("POST", $this->uri . "/api/v1/accounts/" . (int)trim($validDatum[0]) . "/tokenized_payment_method", [
